...
def soft_dice_loss_binary(logits: torch.Tensor, targets: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
...
def topology_branch_recall_loss(logits: torch.Tensor,
    ...
    inter = torch.sum(branch_weight * probs * targets)
    denom = torch.sum(branch_weight * targets) + eps
    recall = (inter + eps) / denom
    return 1.0 - recall


def estimate_terminal_map_from_skeleton(skel: torch.Tensor, dilate_radius: int = 2) -> torch.Tensor:
    if skel.dim() != 4 or skel.shape[1] != 1:
        raise ValueError(f"skel must be [B,1,H,W], got {tuple(skel.shape)}")
...
def terminal_aware_loss(logits: torch.Tensor,
    ...
    inter = torch.sum(terminal_weight * probs * targets)
    denom = torch.sum(terminal_weight * targets) + eps
    recall = (inter + eps) / denom
    return 1.0 - recall
...
class VesselHybridTopologyLossV2(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        loss_dice = soft_dice_loss_binary(logits, targets)
